        ...
        float hardThresh, float softThresh)
{
    ...
    pixel[0][1] = read_imagef(input, sampler, (int2)(2 * x, 2 * y + 1));
    pixel[1][0] = read_imagef(input, sampler, (int2)(2 * x + 1, 2 * y));
    ...
    write_imagef(hl, (int2)(x, y), (col_h[0] + col_h[1]) / 2 + 0.5f);
    write_imagef(lh, (int2)(x, y), (col_l[0] - col_l[1]) / 2 + 0.5f);
