...
__constant float gausssingle[25] = {
    0.6411f, 0.7574f, 0.8007f, 0.7574f, 0.6411f, 0.7574f, 0.8948f, 0.9459f,
    0.8948f, 0.7574f, 0.8007f, 0.9460f, 1.0000f, 0.9460f, 0.8007f, 0.7574f,
    0.8948f, 0.9459f, 0.8948f, 0.7574f, 0.6411f, 0.7574f, 0.8007f, 0.7574f,
    0.6411f
};
...
__kernel void kernel_bilateral_nv12(
    __read_only image2d_t srcYUV, __write_only image2d_t dstYUV,
    float sigma_r, unsigned int imw, unsigned int imh, uint vertical_offset )
{
...
__kernel void kernel_bilateral_rgb(
    __read_only image2d_t srcRGB, __write_only image2d_t dstRGB,
    float sigma_r, unsigned int imw, unsigned int imh)
{
    int x = get_global_id(1);    //[0,imw-1]
    int y = get_global_id(0);    //[0,imh-1]
    int localX = get_local_id(1);    //[0,imw/120-1]
