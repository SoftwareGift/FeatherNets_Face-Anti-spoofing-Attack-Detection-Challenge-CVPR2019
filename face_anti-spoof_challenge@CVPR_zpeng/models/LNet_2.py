...
class LNet(nn.Module):
    def __init__(self,block,layers,num_classes = 2,se = False):
        ...
        self.final_DW = nn.Conv2d(self.channels[4], self.channels[4], kernel_size=3, stride=2, padding=1,
...
#                           kernel_size=3, stride=stride,padding=1, groups=self.inplanes, bias=False),
    ...
    def forward(self, x):
        ...
        if self.se:
            ...
            x = self.layer1(x) # Time: 1.76 ms / iter    FPS: 569.31
            x = self.layer2(x) # Time: 3.59 ms / iter    FPS: 278.30
            x = self.layer3(x) # Time: 6.66 ms / iter    FPS: 150.12
            x = self.layer4(x) # Time: 8.82 ms / iter    FPS: 113.39
#             x = self.layer5(x)
        

#         x = self.conv1x1(x)
        x = self.final_DW(x) # Time: 8.63 ms / iter    FPS: 115.88
# #         print(x.shape)
        x = x.view(x.size(0), -1) # Time: 8.71 ms / iter    FPS: 114.83 Time: 9.20 ms / iter    FPS: 108.74
# #         print(x.shape)
# #         print(type(x))
#         x = self.do(x)
#         x = self.linear(x)
...
#     return model
def LNet54_2_se( **kwargs):
    model = LNet(InvertedResidual, [4, 4, 6, 3], num_classes=2,se = True, **kwargs) # change the downsample (avgpool)
    return model
# def LNet102( **kwargs):
